...
const Header = () => {
  ...
  // To close the dropdown when clicking inside it
      ...
      <div className='flex justify-between items-center pt-3 px-3 md:px-0 md:pr-6'>
        <div className='flex items-center gap-3'>
          ...
          <div className='relative'>
            <button className='md:hidden p-2 rounded-lg bg-indigo-100/80 text-indigo-500' onClick={toggleNavDropdown}>
              {NavDropdownOpen ? (
              ...
              <div className='flex flex-col justify-between gap-2 py-2 items-center bg-white border border-indigo-200/80 rounded-xl h-full'>
                ...
                <ul className='space-y-2'>
                  {[
                    ...
                    <li key={index}>
                      <Link to={item.path} onClick={handleButtonClick} className={`${pathname === item.path ? '!bg-indigo-500 text-white' : ''} bg-indigo-100/70 hover:bg-indigo-200/70 text-indigo-500 transition duration-300 ease-in-out flex items-center justify-center size-12 rounded-full border`}>
                ...
                <button onClick={handleButtonClick} className='bg-red-100/70 hover:bg-red-200/70 text-red-500 transition duration-300 ease-in-out flex items-center justify-center size-12 rounded-full border'>
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="size-6">
        ...
        {/* MOB: User dropdown */}
        <button className='md:hidden p-1 rounded-full bg-indigo-200/50'>
          <div className='flex items-center justify-center size-8 rounded-full bg-indigo-500'>
            <span className='text-white text-sm'>G</span>
          </div>
        </button>
